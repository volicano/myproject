...
//    	}  	
        ...
        //设置分页回调方法
		$show=$page->show();
		$news_list=$news->field(array('id','user_id','user_name','content','addtime','pid'))->order('id desc')->limit($page->firstRow.','.$page->listRows)->select();
    //	$this->filter(&$news_list);
		$news_lists=array();
		$pr=M('Product');	
		foreach ($news_list as $v){
			//print_r($v);
			$pro = $pr->where("Id=".$v['pid'])->find();
			$v['title'] = $pro['Comm_Name'];
			$news_lists[] = $v;
		}		
    	$this->assign('news_list',$news_lists);
